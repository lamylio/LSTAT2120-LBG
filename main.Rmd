...
We see that more than half of the players are European, while the 4 other continents don't exceed 25% individually. We also observe that players that are right-footed are almost four times more than left-footed players.
...
Now we can look at our variable of interest, the wage. It seems that it is not at all following a normal distribution. In fact, we have a a skewness of <code style="font-weight:bold">`r 7.8`</code> which means that it is very right-tailed. Furthermore, it has a kurtosis of <code style="font-weight:bold">103</code> so the \textit{Wage} variable is extremely thick in the tails. We can back up this claim by looking at the histogram and boxplot of the wage. This variable is definitely not normal.

Similarly from the \textit{Wage} variable, we see that the value of the player is also right-skewed and very thick in its tails. Again, we can look at the histogram/boxplot figure of the \textit{Value} variable and we confirm this tendency.
...
Again, this does not mean that we have no multicollinearity at all. We will look at that before model selection in the next section, with more sophisticated tools. 


# Model Selection

We will now start to select variables to include in our model. We will use the two following types of model selection : a stepwise selection using AIC and a LASSO procedure. 
...
We will test the interaction between the \textit{Preferred_Foot} and the \textit{Marking} variables.
...
```{r}
library(car)
hyp = c(0, -8, 1, 0, 1, 1, 1, 1 , 0, 1, 1, 1, 0, 0, 0, 0, 0)
