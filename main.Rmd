...
desc.quantitative.boxplots = function(data, nrow=2, ncol=2, boxframe=T){
  ...
  drawPlots = function(coname){
    ...
    #histo = ggplot(data) + stat_count(aes(x=coname, y=..count..), position = "identity", fill=opt.barcolor) + ggtitle(paste0("Distribution of ", coname))
    #boxp = ggplot(data) + geom_boxplot(aes(y=coname), fill=opt.barcolor) + coord_flip() + xlab("")
