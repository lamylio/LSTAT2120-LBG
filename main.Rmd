...
desc.quantitative.boxplots(dataset, 2, 3, T)
...
```{r}
for(i in c(2:3,5:10, 12:13, 15)){
plot(x = train[,i], y=model$residuals, xlab = colnames(train)[i], ylab = "Residuals")
}
```
...
With the result of the tes above, we reject the normality of the error terms. This could be the result of an inadequate model, in fact this could mean that the error is not random. Like we have already said in the non-linearity section, a complicated non-linearity relation could maybe fix this problem, but it would be out of the scope of this project. 
