...
For this project, we will use a dataset that contains all the FIFA 19 player characteristics (FIFA is a football simulation video game, and our dataset come from the 2019's edition). This dataset contains <code style="font-weight:bold">18059</code> observations (all the different players created in the game) and <code style="font-weight:bold">`r 34`</code> variables (characteristics such as Age, Nationality, Wage, Stamina, ...). 
...
We start by doing a complete case analysis (i.e. eliminating all observations that contain missing values). We can do that without introducing any bias in our analysis, because observations containing missing values constitute an extremely small portion of our dataset (48 of 18207 observations, so less than <code style="font-weight:bold">`r 0.3`%</code>). We then improve the usability of the dataset by changing some units. Initially, the variable \textit{Nationality} that records the nationality of the player was encoded as the country of the player. Since there were too much levels and some of them seemed to be irrelevant, we decided to modify it manually and replace these levels by the associated continents.
...
Among the 34 variables, 27 take values in percentage : it mainly concerns the game features such as agility, dribbling, ball control... The other quantitative variables are the age (given in years), the wage (given in thousands of euros), the height (given in centimeters), the weight (given in kilograms) and the value of the player (given in million euros). We should also emphasize the fact that we have two categorical variables called \textit{Preferred_Foot} (with two levels : Left and Right) and \textit{Nationality} (with five levels : Africa, America, Asia, Europe and Oceania). We can look at the two corrisponding barplots.
...
Let's start with the \textit{Age} variable. Players age have mean <code style="font-weight:bold">25</code>, with a <code style="font-weight:bold">`r 4.7`</code> standard deviation. We see that the variable is skewed right (nothing surprising, some players can still play at 40 but none can play at 5). The kurtosis is slightly less than 3, which means that the variable is maybe a bit light-tailed. 

Now we can look at our variable of interest, the wage. It seems that it is not at all following a normal distribution. In fact, we have a a skewness of <code style="font-weight:bold">`r 7.9`</code> which means that it is very right-tailed. Furthermore, it has a kurtosis of <code style="font-weight:bold">102</code> so the \textit{Wage} variable is extremely thick in the tails. We can back up this claim by looking at the histogram and boxplot of the wage. This variable is definitely not normal.
...
Similarly from the \textit{Wage} variable, we see in the table that the value of the player is also right-skewed and very thick in its tails. The histogram/boxplot figure of the \textit{Value} variable above confirms this tendency.

We can see in the histograms above that, if we take the log of these two variables (<i>Wage and <Value>), we obtain much more suitable distributions. The <Wage> distribution is less agglutinated near zero, and the <Value> distribution seems even normally distributed.

Weight and height of the players seem to be normally distributed (maybe a bit right-skewed for the weight) around <code style="font-weight:bold">`r 181`cm</code> and <code style="font-weight:bold">`r 75`kg</code>, respectively.
...
We have here the upper part of the correlation matrix. We see that we have a triangle of variables that have very high pairwise correlations (lots of those correlations are even superior to <code style="font-weight:bold">`r 0.8`</code>). We are not very surprised of that, because we could expect that good players have globally good abilities and conversely, bad players have on average bad abilities. Thus, those variables contains almost the same information as what we already have in the \textit{Overall} variable. Considering this fact, we decided to discard those variables in order to avoid too much multicollinearity problems. 
...
We will use a stepwise function that finds a model that minimizes AIC, using a step-by-step approach. The AIC criterion is defined with the following formula :
\[AIC = 2k-2ln(L)\]
where $k$ is the number of parameter to estimate and $L$ is the likelihood function of the model.
As a reminder, the AIC, just like the adjusted R-squared, is a measure of the quality of the model that penalizes the addition of new variables. This is what we need in order to do a stepwise regression. The method will check whether the AIC keeps decreasing while we include more and more variables. 
...
model.aiclogy = stepAIC(model.complete.logy, direction = "both", trace = F)
...
In the stepwise summary in the appendix we see that the selection discards 6 variables : \textit{Age}, \textit{Height}, \textit{Weight}, \textit{Jumping}, \textit{Marking} and \textit{Strength}.  
...
Indeed, for a certain coefficient t>0 to be determined, we impose that \[ \sum_{j=1}^{p} |\beta_j| \leq t \] We can see the results in the appendix.
...
We have now looked at 6 different models : the complete regression model (two kinds), the AIC-stepwise regression model (two kinds) and the LASSO regression model (two kinds). Indeed, for each of them, we have estimated a model of regression against the \textit{Wage} variable and against the \textit{log_wage} variable. We have done that because we suspected non linear relations in our model. 
```{r}
formattable(models.comparison, list(
  formattable::area(row=4) ~ formatter("span", style= ~ style(font.weight="bold"))
)) 
```

The table above shows a model comparison for those 6 models. We use various criteria such as the log-likelihood, the AIC, the adjusted R-squared and the deviance. We see that the adjusted R-squared is slightly better if we don't take the log of the response variable, but all the other criteria are far much better when we take it. We decide to take as final model the AIC-stepwise model with log of the \textit{Wage} variable because it has almost as good goodness of fit criteria as the LASSO, but it has only 15 variables (compared to 20 for the LASSO regression). The final is then given by \[\log{Wage} = -7.96 + 0.11 Overall + 0.018Potential + 0.0021Weight + 0.005HeadingAccuracy + 0.0034Reactions \\ - 0.0008Jumping - 0.0035Strength + 0.002Aggression + 0.0018Composure - 0.003 Marking + 0.028Value \\ + 0.104NationalityAfrica + 0.284NationalityAsia + 0.117NationalityEurope + 0.021Preferred\_FootLeft\]
where \textit{NationalityAfrica}, \textit{NationalityAsia}, \textit{NationalityEurope} and \textit{Preferred_FootLeft} are dummy variables.
```{r}
#setting the final model (aic logy)
model = model.aiclogy
  
# lm(log(Wage) ~ Overall + Potential + Weight + HeadingAccuracy + Reactions + Jumping + Strength + Aggression + Composure + Marking + Value + NationalityAfrica + NationalityAsia + NationalityEurope + Preferred_FootLeft, data = train)

```

## Interactions

We test different possible interactions in the following : we add each time an interaction term in the basis model (final model) and then look at the properties of the new model. The first model with interaction contains an interaction term between the \textit{Preferred_FootLeft} variable and the \textit{Marking} variable. We then consider interaction between \textit{Preferred_FootLeft} and \textit{Value} (Model 2 with interaction), interaction between \textit{NationalityEurope} and \textit{Potential} (Model 3 with interaction) and interaction between \textit{NationalityAfrica} and \textit{Value} (Model 4 with interaction).
...
Using the criteria described previously, we obtain <code style="font-weight:bold">`r  outliers_X`</code> outliers with respect to X (about <code style="font-weight:bold">`r round(outliers_X/nrow(explanatory), 3)*100`%</code> of the players). 
...
t = ols_plot_resid_stud_fit(model, print_plot = T)
t$plot$layers[[3]] = NULL;t$plot$layers[[3]] = NULL;
```
Again, our decision rule allows us to measure the number of outliers with respect to Y. We have <code style="font-weight:bold">`r outliers_Y`</code> outliers of that kind out of <code style="font-weight:bold">`r nrow(explanatory)`</code> players, which is quite low again (about <code style="font-weight:bold">`r round(outliers_Y/nrow(explanatory), 3)*100`%</code>). Since $t_{14309;0.975} \approx 2$, under our criteria, all the outliers with respect to Y are represented in red on the previous plot. 
...
```{r}
plot(cooks.distance(model), xlab="Index of the observation", ylab="Cooks distance", pch=21, bg = opt.barcolor)
...
We see on the result table above that we have no VIF superior to 10, which is good news. The mean of all these VIF is of <code style="font-weight:bold">2.44</code>. Since it is not extremely far from one, we decide not to take remedial actions such as a Ridge Regression. 
...
```{r}
#Residuals vs fitted
plot(model$residuals, model$fitted.values, xlab="Residuals", ylab="Fitted Values", pch=21, bg=opt.barcolor)
```
...
The variable \textit{Weight}, \textit{Jumping} and \textit{Preferred_Foot} seem to be not significant at level <code style="font-weight:bold">0.05</code>. This means that for these 3 variables, we can not reject the null hypothesis that the corresponding coefficient is null. Therefore, we will abstain ourselves from interpreting those coefficients. We note that all the coefficients estimates $\beta_j$ have a positive sign, apart from \textit{Strength} and \textit{Marking}. It seems normal that our continuous variables has a positive sign. Indeed, most of them are outlining player abilities, and it is clear that players who have better capabilities are on average better paid. However, marking and strength abilities seem to lower the salary, and we can not think of an explanation for that phenomenon. We can now take a look more precisely at our qualitative variables (\textit{Nationality} and \textit{Preferred_Foot}). We see that the fact to be left-footed raise on average your salary by 1000\$. Since left-footed players compose only one fifth of all players, managers may want to keep them in the team, and therefore pay them more then the others. For the \textit{Nationality} variable, the Asian Nationality 
...
We will now test if a linear combination of our coeficients holds. We are interested in the effect of the \textit{Overall} variable. The objective will be to see if its effect is compensated by the effect of the other coefficients relative to player abilities. We will therefore test the following hypothesis : 
\[H_0 : \beta_{Overall} 
...
We see in the table above that we can reject the restricted model (i.e. the model under $H_0$). We can conclude that the effect of the \textit{Overall} variable is not equal to the mean of the effect of the players abilities. 
...
legend("topright", legend = c("Observed Values", "Predicted Values"), fill = c("black", opt.barcolor))
points(1:nrow(valid), predictions[,1], col=opt.barcolor, pch=16, cex=0.1)
...
We can see above the actual values of the \textit{Wage} variable of the validation dataset, next to the predicted ones. We see a certain fit with the observed data, which is good. However, we calculate that only <code style="font-weight:bold">`r round(k,2)`%</code> of our prediction intervals contains the observed values, which is very poor. 
