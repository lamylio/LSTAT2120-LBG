...
outliers_Y=0
for(i in 1:14326){
  if(abs(as.matrix(rstudent(model = lm(log(Wage) ~ Overall + Potential + Weight + HeadingAccuracy + 
    Reactions + Jumping + Strength + Aggression + Composure + 
    Marking + Value + NationalityAfrica + NationalityAsia + NationalityEurope + 
    Preferred_FootLeft, data = train)))[i])>qt(0.975, 14309)){
...
for(i in 1:14326){
  if(abs(as.matrix(rstudent(model = lm(log(Wage) ~ Overall + Potential + Weight + HeadingAccuracy + 
    Reactions + Jumping + Strength + Aggression + Composure + 
    Marking + Value + NationalityAfrica + NationalityAsia + NationalityEurope + 
    Preferred_FootLeft, data = train)))[i]*sqrt(as.matrix(explanatory[i,])%*%matrix.inverse(crossprod(as.matrix(explanatory),as.matrix(explanatory)))%*%as.matrix(t(explanatory[i,]))/(1-as.matrix(explanatory[i,])%*%matrix.inverse(crossprod(as.matrix(explanatory),as.matrix(explanatory)))%*%as.matrix(t(explanatory[i,])))))>2*sqrt(16/14326)){
...
```{r}
#Residuals vs fitted
plot(model$fitted.values, model$residuals, col=opt.barcolor, 
...
\[H_0 = \beta_{Overall} = \frac{\beta_{Potential}+\beta_{HeadingAccuracy}+\beta_{Reactions}+\beta_{Jumping}+\beta_{Strength}+
