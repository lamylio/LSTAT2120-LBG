...
outliers_X=0
for(i in 1:14326){
if(as.matrix(explanatory[i,])%*%matrix.inverse(crossprod(as.matrix(explanatory),as.matrix(explanatory)))%*%as.matrix(t(explanatory[i,]))>32/14326){
  outliers_X=outliers_X+1
}
}
cat("The number of outliers with respect to X is:")
outliers_X
```
...
outliers_Y=0
for(i in 1:14326){
  if(abs(as.matrix(rstudent(model))[i])>qt(0.975, 14309)){
    outliers_Y=outliers_Y+1
    }
}
cat("The number of outliers with respect to Y is:")
outliers_Y
...
```{r}
influential_fitted=0
for(i in 1:14326){
  if(abs(as.matrix(rstudent(model))[i]*sqrt(as.matrix(explanatory[i,])%*%matrix.inverse(crossprod(as.matrix(explanatory),as.matrix(explanatory)))%*%as.matrix(t(explanatory[i,]))/(1-as.matrix(explanatory[i,])%*%matrix.inverse(crossprod(as.matrix(explanatory),as.matrix(explanatory)))%*%as.matrix(t(explanatory[i,])))))>2*sqrt(16/14326)){
    influential_fitted=influential_fitted+1
    }
}
influential_fitted
